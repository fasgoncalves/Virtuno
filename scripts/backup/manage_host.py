...
def send_alarm():
    ...
    TOPIC="Alertas"
    MESSAGE = "⚠️ Alerta : Humano detectado! Imagem Movimento Humano no exterior! [ " + http_path + " ]"
    URL = f"http://softelabs.pt:9000/{TOPIC}"
    USER = "fgoncalves"
    PASSWORD = "vu#Al1"
    ...
    logging.info("Enviada Mensagem com link para imagem alarmada " + env_path + " Status: " + str(response))
    return response
#
...
def monitor_page():
    ...
    with ui.row().classes("w-full justify-between items-center bg-gray-200 p-3 shadow-md text-blue"):
        with ui.row().classes('items-center justify-between w-full p-2'):
           ui.label("📊 KVM HOST MONITOR").classes("text-lg font-bold")
